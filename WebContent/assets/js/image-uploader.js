...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        // Se añade la configuracion del toast para los mensajes.
        toastr.options = {
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                ...
                msg = (`El archivo "${file.name}" no coincide con las extensiones de archivo aceptadas: "${plugin.settings.extensions.join('", "')}"`);

                $(document).Toasts('create', {
                    class: 'bg-warning',
                    title: 'Tamaño max superado',
                    body: msg
                })
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                msg = (`El archivo "${file.name}" no coincide con los tipos de formato de imagen aceptados: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                msg = (`La imagen "${file.name}" excede el tamaño máximo de: ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                msg = (`La imagen "${file.name}" no pudo ser añadido porque se alcanzó el límite de archivos: ${plugin.settings.maxFiles}`);
                // toastr.warning(msg);

                $(document).Toasts('create', {
                    class: 'bg-warning',
                    title: 'Max. imagenes superado',
                    body: msg
                })
                return false;
            }
